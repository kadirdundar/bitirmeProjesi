...
class savedVC: UIViewController {
    ...
    func verileriIsle(){
        ...
        /*var matchingDocuments = [[DocumentSnapshot]](repeating: [], count: kumeSayisi)
       ...
       for i in 0..<kumeSayisi {
            for location in unscaleData[i] {
                ...
                query.getDocuments { (snapshot, error) in
                    if let error = error {
                        ...
                        for document in snapshot!.documents {
                            matchingDocuments[i].append(document)
                            print(matchingDocuments[i])
                        }
                    }
                }
            }
        ...
        for i in 0..<matchingDocuments.count {
            ...
            for j in 0..<matchingDocuments[i].count {
                ...
                let documentRef = Firestore.firestore().collection("information").document(documentID)
                        batch.updateData(updateData, forDocument: documentRef)
                        }
                        }
                ...
                docRef.getDocuments { (querySnapshot, error) in
                    ...
                    for document in documents {
                        let documentID = document.documentID
                                        Firestore.firestore().collection("information").document(documentID).updateData(["arac": value])
                                        print(value)
